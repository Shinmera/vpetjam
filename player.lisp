...
(define-shader-entity player (part-parent playhead facing-entity game-entity alloy:observable-object)
  ...
  (:default-initargs :children '((:head  :uv (0 7) :pivot (0 -18) :children ((:face :uv (0 1) :location (0 -2 -1))))
                                 (:lleg  :uv (0 5) :pivot (0  28) :children ((:foot :uv (0 4) :location (0 -30 +1))))
                                 (:rleg  :uv (0 5) :pivot (0  28) :children ((:foot :uv (0 4) :location (0 -30 +1))))
...
(define-clip player-stand-forward
  0.0 (((:head location) (vec   0  50 -1)) ((:head angle) 0.0) ((:head skew) (vec +1 +1))
       ((:lleg location) (vec -10 -58 +1)) ((:lleg angle) 0.0) ((:lleg skew) (vec +1 +1))
       ...
       (((:head :face) location) (vec 0 -2 -1))
       (((:lleg :foot) location) (vec 0 -30 +1))
       (((:rleg :foot) location) (vec 0 -30 +1)))
...
(define-clip player-stand-side
  0.0 (((:head location) (vec  +3  55 -1)) ((:head angle) 0.0) ((:head skew) (vec +1 +1))
       ((:lleg location) (vec  -5 -60 -1)) ((:lleg angle) 0.0) ((:lleg skew) (vec +1 +1))
       ((:rleg location) (vec  +5 -45 +1)) ((:rleg angle) 0.0) ((:rleg skew) (vec +1 +1))
       ((:larm location) (vec  -8  -5 -2)) ((:larm angle) +0.2) ((:larm skew) (vec +1 +1))
       ((:rarm location) (vec  +2  +5 +1)) ((:rarm angle) -0.2) ((:rarm skew) (vec +1 +1))
       (((:head :face) location) (vec 2 -2 -1))
       (((:lleg :foot) location) (vec -10 -30 +1))
       (((:rleg :foot) location) (vec -10 -30 +1)))
  1.0 (((:head location) (vec  +3  50 -1)) ((:larm angle) -0.2) ((:rarm angle) +0.2))
  2.0 (((:head location) (vec  +3  55 -1)) ((:larm angle) +0.2) ((:rarm angle) -0.2)))

(define-clip player-walk-side
  0.0 (((:head location) (vec  +3  50 -1)) ((:head angle) 0.0) ((:head skew) (vec +1 +1))
       ((:lleg location) (vec  -5 -60 -1)) ((:lleg angle) -0.75) ((:lleg skew) (vec +1 +0.7))
       ((:rleg location) (vec  +5 -45 +1)) ((:rleg angle) +0.75) ((:rleg skew) (vec +1 +1))
       ((:larm location) (vec  -8  -5 -2)) ((:larm angle) +0.6) ((:larm skew) (vec +1 +1))
       ((:rarm location) (vec  +2  +5 +1)) ((:rarm angle) -0.6) ((:rarm skew) (vec +1 +1))
       (((:head :face) location) (vec 2 -2 -1))
       (((:lleg :foot) location) (vec -10 -30 +1))
       (((:rleg :foot) location) (vec -10 -30 +1)))
  0.2 (((:rleg angle)  0.0) ((:rleg skew) (vec +1 +1.0))
       ((:head location) (vec  +3  55 -1)))
  0.5 (((:lleg angle) +0.75)
...
(define-clip player-pickup-forward
  0.0 (((:larm location) (vec -20   0 -2)) ((:larm angle) +0.1) ((:larm skew) (vec +1 +1))
       ((:rarm location) (vec +20   0 -2)) ((:rarm angle) +0.1) ((:rarm skew) (vec -1 +1)))
  0.3 (((:larm location) (vec -20   0 -2)) ((:larm angle) +0.1) ((:larm skew) (vec +1 -1))
       ((:rarm location) (vec +20   0 -2)) ((:rarm angle) +0.1) ((:rarm skew) (vec -1 -1))))

(define-clip player-pickup-backward
  0.0 (((:larm location) (vec -20   0 +2)) ((:larm angle) +0.1) ((:larm skew) (vec +1 +1))
       ((:rarm location) (vec +20   0 +2)) ((:rarm angle) +0.1) ((:rarm skew) (vec -1 +1)))
  0.3 (((:larm location) (vec -20   0 +2)) ((:larm angle) +0.1) ((:larm skew) (vec +1 -1))
       ((:rarm location) (vec +20   0 +2)) ((:rarm angle) +0.1) ((:rarm skew) (vec -1 -1))))
...
(defmethod handle :after ((ev tick) (player player))
  (let ((vel (velocity player)))
    ...
    (setf (clip (action-playhead player))
          (when (stack player)
            (cond ((/= 0 (vx (direction player)))
                   'player-pickup-side)
                  ((< 0 (vy (direction player)))
                   'player-pickup-backward)
                  (T
                   'player-pickup-forward))))
...
(defmethod handle ((ev pick-up) (player player))
  (let ((nearest NIL)
        (loc (location player))
        (held (first (stack player))))
    (bvh:do-fitting (entity (bvh +world+) (vec4 (- (vx loc) 128) (- (vy loc) 128)
      ...
      (when (and (not (eq player entity))
        ...
        (setf nearest entity)))
    (when nearest
      (pick-up nearest player)
      T)))

(defmethod handle ((ev interact) (player player))
  (let ((held (first (stack player)))
        (cursor (cursor player))
        (loc (location player)))
    (if (and held (target cursor) (object-accepted-p held (target cursor)))
        (receive (pop (stack player)) (target cursor))
        (or (bvh:do-fitting (entity (bvh +world+) (vec4 (- (vx loc) 128) (- (vy loc) 128)
                                                        (+ (vx loc) 128) (+ (vy loc) 128)))
              (when (and (<= (vlength (velocity player)) 1.0)
                ...
                (loop for held = (pop (stack player))
                      while (object-accepted-p held entity)
              ...
              (when (and (typep entity 'interactable)
                ...
                (return T)))
            (handle (make-instance 'pick-up) player)
            (handle (make-instance 'drop) player)))))
